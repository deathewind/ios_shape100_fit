...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        weix_pay.frame = CGRectMake(0, label_pay.frame.size.height, self.frame.size.width, 44);
        ...
        alipay.frame = CGRectMake(0, weix_pay.frame.size.height + weix_pay.frame.origin.y, self.width, weix_pay.height);
        ...
        UIImageView *line3 = [[UIImageView alloc] initWithFrame:CGRectMake(0, weix_pay.height - 0.5, self.width, 0.5)];
        line3.backgroundColor = [UIColor lightGrayColor];
        [alipay addSubview:line3];
    }
    return self;
}
- (void)addSubviewBy:(NSString *)imageName payName:(NSString *)payName color:(UIColor *)color button:(UIButton *)view{
   ...
   // CGSize  textSize = [payName sizeWithFont:YXCharacterBoldFont(16) constrainedToSize:CGSizeMake(200, 100) lineBreakMode:NSLineBreakByWordWrapping];
    CGSize textSize = [payName boundingRectWithSize:CGSizeMake(200, 100) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:YXCharacterFont(16)} context:nil].size;
